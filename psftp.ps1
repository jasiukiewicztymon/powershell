    ...
    switch ($choice) {
        1 {
            ...
            if ($save -eq "y") {
                ...
                $securehostname = ConvertTo-SecureString -String $hostname -AsPlainText -Force
                ...
                $secureunpassword = ConvertTo-SecureString -String $unpassword -AsPlainText -Force
        ...
        2 {
            ...
            [array]$strkey = $profilpassword.Split(",") 
            try  { [array]$key = foreach($number in $strkey) {([int]::parse($number))} }
            catch { Write-Warning -Message "Wrong key syntax" }
            ...
            if ($strkey.Length -eq 24) {
                ...
                Write-Warning -Message "Wrong key syntax"
            }          
...
#108,7,27,13,2,44,121,38,95,61,71,66,117,37,119,100,47,5,115,148,122,4,69,95
